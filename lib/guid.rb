...
class Guid
  ...
  def Guid.for(prefix)
    # clean_prefix = (prefix.nil?) ? 'UU' : prefix.to_s[0,2].upcase
    # lowercase prefix are handy 
    clean_prefix = (prefix.nil?) ? 'UU' : prefix.to_s[0,2]
    guid = "#{clean_prefix}-#{Guid.new.to_base36}"
    return guid
  end
